...
round_max = 100
two_buffer = False # Whether use two buffer to sample data

### DDPG parameter setting
gamma, tau = 0.99, 1e-3

### Create batch data
machine_num, job_num = 4, 20
...
while((round_ins <= 100) and (early_stop != True)):
    ins = random.sample(range(start, end+1, 1), 1)[0]
    round_ins += 1
    _tStart = time.time()   
    print("=======================================")
...
Range = 100 if len(action_list) <= 1000 else 200 if len(action_list) <= 2000 \
        ...
        else 900 if len(action_list) <= 9000 else 1000
...
print('The size is %s, and trained by one instance' % (size))
# ============================================================================
